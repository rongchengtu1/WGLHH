...
def calc_p_r_hamming(qB, rB, queryL, retrievalL, h=2):
    num_query = queryL.shape[0]
    h1 = h + 0.5
    # print('++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    p_hamming = 0.
    ...
    for iter in range(num_query):
        ...
        tgnd = gnd[0:total]
        ture_point = tgnd.sum()
        p_hamming += ture_point / total
        r_hamming += ture_point / tsum

    p_hamming /= num_query
    r_hamming /= num_query
    return p_hamming, r_hamming
if __name__=='__main__':
    for name in ['DHN', 'DPSH', 'DSH', 'Hashnet', 'DSDH']:
        r = []
        p = []
        for i in [24, 32, 48, 64]:
            with open('/data/home/trc/SMH/DeepHash-pytorch-master/codes/' + name + '_coco_code_' + str(i) + '.pkl',
                      'rb') as f:
                B_L = pickle.load(f)
            qB_img = np.asarray(B_L['qB'])
            qf = np.asarray(B_L['qf'])
            rB = np.asarray(B_L['dB'])
            rf = np.asarray(B_L['df'])
            test_labels = np.asarray(B_L['query_L'])
            database_label_single = np.asarray(B_L['retrieval_L'])
            num_database = database_label_single.shape
            map = calc_HammingMap(qB_img, rB, qf, rf, test_labels, database_label_single, 2)
            print(name + '_' + str(i) + 'bit_MAP2:', map)
            current_p, current_r = calc_p_hamming(qB_img, rB, test_labels, database_label_single, 2)
            p.append(current_p)
            r.append(current_r)
        print('precision:', p)
        print('recall:', r)
